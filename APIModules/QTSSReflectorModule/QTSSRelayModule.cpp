    ...
    for (OSQueueIter iter(&theSessionQueue); !iter.IsDone(); iter.Next())
    {
        ...
        for (OSQueueIter iter2(RelayOutput::GetOutputQueue()); !iter2.IsDone(); iter2.Next())
        {
            ...
            if (theSession == theOutput->GetRelaySession())
            {
                ...
                //qtss_sprintf(theStatsBuf, "Current stats for this relay: %lu packets per second. %lu bits per second. %I64d packets since it started. %I64d bits since it started<P>", theOutput->GetCurPacketsPerSecond(), theOutput->GetCurBitsPerSecond(), theOutput->GetTotalPacketsSent(), theOutput->GetTotalBytesSent());
                qtss_sprintf(theStatsBuf, "Current stats for this relay: %lu packets per second. %lu bits per second. %I64d packets since it started. %I64d bits since it started<P>", theOutput->GetCurPacketsPerSecond(), theOutput->GetCurBitsPerSecond(), theOutput->GetTotalPacketsSent(), theOutput->GetTotalBytesSent());   //_andy_
